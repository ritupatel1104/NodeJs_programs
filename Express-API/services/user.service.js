...
module.exports.forgetPassword = async (email) => {
    ...
    // created reset token expiry time = generate link data and time + extra 15 min
    // token will be validate only 15 minutes, after minutes token will be expiry andcan't reset your password
 await user.save()    ;

 const resetLink = `http://localhost:3002/reset-password/${token}`
 //reset link = frontend page link that show newpassword field with change password button
...
module.exports.resetPassword = async ({token,
     newPassword }) =>{
